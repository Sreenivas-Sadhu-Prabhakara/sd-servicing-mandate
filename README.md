...
| **Asset Type** | Servicing Mandate |
| **Control Record** | Servicing Mandate Management Plan |
...
| POST | `/v1/servicing-mandate-management-plan/initiate` | Initiate |
| GET | `/v1/servicing-mandate-management-plan` | Retrieve (list) |
| GET | `/v1/servicing-mandate-management-plan/{crId}/retrieve` | Retrieve |
| PUT | `/v1/servicing-mandate-management-plan/{crId}/update` | Update |
| PUT | `/v1/servicing-mandate-management-plan/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/servicing-mandate-management-plan/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-servicing-mandate:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-servicing-mandate ./helm -n bian-sales-service
```

Exposed through the platform Gateway at path prefix `/sd-servicing-mandate` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
